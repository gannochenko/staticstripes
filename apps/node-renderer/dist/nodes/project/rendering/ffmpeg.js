...
exports.makeKenBurns = makeKenBurns;
 ...
 */
function makeFFmpegCommand(assetIndexMap, assets, output, filterComplex, ffmpegArgs) {
    const parts = ['ffmpeg'];
    ...
    const missingAssets = [];
    for (const [assetName, index] of assetIndexMap) {
    ...
    if (missingAssets.length > 0) {
        throw new Error(`Filter graph references assets that don't exist:\n${missingAssets.map((a) => `  - ${a}`).join('\n')}\n\n` +
    ...
    // Add FFmpeg arguments (encoding parameters, codecs, etc.)
    if (ffmpegArgs) {
...
function makeFormat(inputs, format) {
    if (inputs.length !== 1) {
        throw new Error(`makeFormat: expects one input`);
    }
    const input1 = inputs[0];
    if (input1.isAudio) {
        throw new Error(`makeFormat: input1 must be video, got audio (tag: ${input1.tag})`);
    }
    const output = {
        tag: (0, label_generator_1.getLabel)(),
        isAudio: false,
 ...
 */
function makeKenBurns(inputs, options) {
    const input = inputs[0];
    if (input.isAudio) {
        throw new Error(`makeKenBurns: input must be video, got audio (tag: ${input.tag})`);
    }
    const output = {
        tag: (0, label_generator_1.getLabel)(),
        isAudio: false,
    ...
    const getEasingExpr = (easing, prog) => {
        switch (easing) {
    ...
    const swExpr = (z) => `floor(${W}*(${z})/2)*2`;
    const shExpr = (z) => `floor(${H}*(${z})/2)*2`;
    let filterStr;
