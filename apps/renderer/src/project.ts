...
/**
 * Convenience function for parsing HTML project definitions
 * Creates an HTMLProjectParser instance and parses the project
 */
export async function parseHTMLDefinition(
  html: ParsedHtml,
  projectPath: string,
): Promise<Project> {
  const parser = new HTMLProjectParser(html, projectPath);
  return parser.parse();
}
