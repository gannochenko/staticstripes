...
glue_glitch_youtube_bg_intro:
	...
	TOTAL_DUR=$$(echo "$$DURATION * 2 + 5 + 3" | bc); \
	FADE_START=$$(echo "$$DURATION * 2 + 5 - 1" | bc); \
	...
	ffmpeg -y -loop 1 -t 5 -i ./images/20251224_110757.jpg -i ./input/20251224_110901.mp4 -i ./input/20251224_110901.mp4 -i ./effects/digital_glitch_01.mp4 -i ./audio/instrumental-acoustic-guitar-music-401434.mp3 -f lavfi -t 3 -i color=c=black:s=1920x1080:r=30 -f lavfi -t 3 -i anullsrc=r=48000:cl=stereo -i ./images/heart_hands.png -filter_complex \
	  "[0:v]fps=30,format=yuv420p,scale=-1:1920[intro_scaled]; \
	   ...
	   [vcomposed][glitch]overlay=(W-w)/2:0:enable='between(t,$$OFFSET,$$OFFSET+$$GLITCH_DUR)',fade=t=out:st=$$FADE_START:d=1[vmain_faded]; \
	   [7:v]scale=80:-1[emoji]; \
	   [5:v]drawtext=text='Thanks for watching! ':font='Arial':fontsize=64:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2[outro_text]; \
	   [outro_text][emoji]overlay=x=w/2+360:y=h/2-40[outro]; \
	   [vmain_faded][outro]concat=n=2:v=1:a=0[v]; \
	   [4:a]atrim=0:5,afade=t=out:st=4:d=1[music]; \
	   [1:a][2:a]concat=n=2:v=0:a=1[amain]; \
	   [music][amain]concat=n=2:v=0:a=1,afade=t=out:st=$$FADE_START:d=1[amain_faded]; \
	   [amain_faded][6:a]concat=n=2:v=0:a=1[a]" \
	  -map "[v]" -map "[a]" -c:v libx264 -crf 20 -preset fast test_output.mp4

glue_glitch_youtube_bg_intro_preview:
	@DURATION=$$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ./input/20251224_110901.mp4); \
	GLITCH_DUR=$$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ./effects/digital_glitch_01.mp4); \
	OFFSET=$$(echo "$$DURATION - $$GLITCH_DUR / 2 + 5" | bc); \
	TOTAL_DUR=$$(echo "$$DURATION * 2 + 5 + 3" | bc); \
	...
	ffmpeg -y -loop 1 -t 5 -i ./images/20251224_110757.jpg -i ./input/20251224_110901.mp4 -i ./input/20251224_110901.mp4 -i ./effects/digital_glitch_01.mp4 -i ./audio/instrumental-acoustic-guitar-music-401434.mp3 -f lavfi -t 3 -i color=c=black:s=1920x1080:r=30 -f lavfi -t 3 -i anullsrc=r=48000:cl=stereo -i ./images/heart_hands.png -filter_complex \
	  "[0:v]fps=30,format=yuv420p,scale=-1:1920[intro_scaled]; \
	   ...
	   [music][amain]concat=n=2:v=0:a=1,afade=t=out:st=$$FADE_START:d=1[amain_faded]; \
	   [amain_faded][6:a]concat=n=2:v=0:a=1[a]" \
	  -map "[v]" -map "[a]" -c:v libx264 -crf 28 -preset ultrafast test_output.mp4

glue_glitch_youtube_bg_intro_test_outro:
	@DURATION=$$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ./input/20251224_110901.mp4); \
	GLITCH_DUR=$$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ./effects/digital_glitch_01.mp4); \
	OFFSET=$$(echo "$$DURATION - $$GLITCH_DUR / 2 + 5" | bc); \
	TOTAL_DUR=$$(echo "$$DURATION * 2 + 5 + 3" | bc); \
